...
public abstract class CommonRecyclerAdapter<T> extends RecyclerView.Adapter<CommonRecyclerAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        if (position < mHeaders.size()) {
            ...
            T data = mDatas.get(position - mHeaders.size());
            holder.baseViewHolder.updateView(data, position - mHeaders.size());

            if (data instanceof ExposeItem) {
                ExposeItem exposeItem = (ExposeItem) data;
                if (!exposeItem.isTracked()) {
                    holder.baseViewHolder.onExpose(data, position - mHeaders.size());
