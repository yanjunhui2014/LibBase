...
public class FZSystemBarUtils {
    ...
    public static void notImmersiveStatusBar(Activity activity) {
        notImmersiveStatusBar(activity.getWindow());
    }

    public static void notImmersiveStatusBar(Window window) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
    ...
    public static void setStatusBarDarkMode(Window window) {
        ...
        } else if (isMIUI6Later() && !isDevMIUI()) {
    ...
    public static boolean isMIUI6Later() {
        try {
            ...
            int versionInt = Integer.parseInt(val);
            return versionInt >= 6 && versionInt < 9;
        } catch (Exception e) {
            return false;
        }
    }

    public static boolean isDevMIUI() {
        try {
            Class<?> clz = Class.forName("android.os.SystemProperties");
            Method mtd = clz.getMethod("get", String.class);
            String version = (String) mtd.invoke(null, "ro.build.version.incremental");
            return version != null && !version.contains("MXDCN");
        } catch (Exception e) {
            return false;
        }
    }
